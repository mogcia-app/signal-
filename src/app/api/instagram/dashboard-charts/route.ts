...
async function generateAIRecommendations(
  userId: string,
  followerGrowthData: Array<{ date: string; growth: number; dayName: string }>,
  postFrequencyData: Array<{ date: string; count: number; dayName: string }>
) {
  try {
    // フォロワー成長の分析
...
async function analyzeOptimalPostingTime(userId: string) {
  try {
    ...
    analyticsSnapshot.docs.forEach((doc) => {
      ...
      const engagement =
        (Number(data.likes) || 0) + (Number(data.comments) || 0) + (Number(data.shares) || 0);
...
async function generateHashtagSuggestions(userId: string) {
  try {
    ...
    const nicheTags =
      popularHashtags.length > 0 ? popularHashtags : ["#B2C", "#コミュニティ", "#成長", "#テスト"];
...
function generateContentStrategy(
  ...
  if (growthTrend === "up") {
    improvement =
      "フォロワー成長が順調です。現在の戦略を継続し、より多くのエンゲージメントを促進しましょう。";
  } else if (growthTrend === "down") {
    improvement =
      "フォロワー成長が鈍化しています。コンテンツの質を向上させ、より魅力的な投稿を心がけましょう。";
  } else {
    improvement =
      "フォロワー成長が安定しています。新しいコンテンツタイプを試して成長を加速させましょう。";
  }
