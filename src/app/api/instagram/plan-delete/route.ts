...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // PlanRepositoryのdeletePlanメソッドを使用（トランザクションで実行）
    ...
    return NextResponse.json(
      { error: errorMessage },
      { status: error instanceof Error && errorMessage.includes("見つかりません") ? 404 : 
               error instanceof Error && errorMessage.includes("権限") ? 403 : 500 }
    );
  }
}
