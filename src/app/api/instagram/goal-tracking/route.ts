  ...
  try {
    ...
    if (!goalSettings) {
      return NextResponse.json({
        success: true,
        data: { goals: [] },
        message: "No goal settings data found",
      });
    }
    ...
    const goals = [
      {
        ...
        percentage: goalSettings.weeklyPostGoal
          ? Math.round((weeklyPostCount / goalSettings.weeklyPostGoal) * 100)
          : 0,
      },
      {
        title: "フォロワー増加",
        ...
        unit: "人",
        status:
          totalFollowerIncrease >= (goalSettings.followerGoal || 0) ? "achieved" : "in_progress",
        percentage: goalSettings.followerGoal
          ? Math.round((totalFollowerIncrease / goalSettings.followerGoal) * 100)
          : 0,
      },
      {
        title: "月間投稿目標",
        ...
        unit: "件",
        status:
          monthlyPostCount >= (goalSettings.monthlyPostGoal || 0) ? "achieved" : "in_progress",
        percentage: goalSettings.monthlyPostGoal
          ? Math.round((monthlyPostCount / goalSettings.monthlyPostGoal) * 100)
          : 0,
      },
    ];
