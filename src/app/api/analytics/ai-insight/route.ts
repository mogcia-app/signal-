...
【重要】分析のアプローチについて:
- 投稿の件数（postCount）を言及する必要はありません。ユーザーは1つずつ投稿の分析データを入力しているため、件数ではなく個別の投稿データ（recentPosts）の内容や傾向に焦点を当ててください
- 解析対象データに記載されている数値（totals、averages、recentPosts内の各投稿の数値など）を必ず正確に参照してください
- データに記載されていない数値や、データと矛盾する表現は絶対に避けてください
- 例えば、totals.reachが5000の場合、「リーチが4千」などと書かないでください
- recentPostsに含まれる各投稿の具体的な数値（likes、comments、reach、engagementRateなど）を正確に使用してください
- 個別の投稿の特徴やパフォーマンスの傾向、成功パターンや改善ポイントを分析してください
