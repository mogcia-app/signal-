...
export async function GET(request: NextRequest) {
  try {
    ...
    let snapshot;
    try {
      snapshot = await adminDb.collection("notifications").where("status", "==", "published").get();

      console.log("✅ Firestoreクエリ成功:", { docCount: snapshot.docs.length });
    } catch (firestoreError) {
      console.error("❌ Firestoreクエリエラー:", firestoreError);
      throw new Error(
        `Firestoreクエリエラー: ${firestoreError instanceof Error ? firestoreError.message : "Unknown error"}`
      );
    }

    let firestoreNotifications: Notification[] = [];
    try {
      firestoreNotifications = snapshot.docs.map(
        (doc) =>
          ({
            id: doc.id,
            ...doc.data(),
      ...
      throw new Error(
        `データ変換エラー: ${mappingError instanceof Error ? mappingError.message : "Unknown error"}`
      );
    }
    ...
    if (userId !== "current-user") {
      filteredNotifications = filteredNotifications.filter(
        (n) => !n.targetUsers || n.targetUsers.length === 0 || n.targetUsers.includes(userId)
      );
