      ...
      <div className="bg-white rounded-none shadow-sm border border-gray-200 p-6">
        <div className="flex items-center mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-black">投稿分析ページで入力した実際の投稿時間ベースの分析</p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {timeSlotData.map(({ label, color, postsInRange, avgEngagement }) => (
            <div
              key={label}
              className={`p-3 rounded-none ${postsInRange > 0 ? "bg-gray-50" : "bg-gray-25"}`}
            >
              ...
              {postsInRange > 0 ? (
                ...
                <div className="text-center py-2">
                  <div className="text-xs text-black italic">📅 この時間帯はまだ投稿なし</div>
                </div>
              )}
            </div>
          ))}
