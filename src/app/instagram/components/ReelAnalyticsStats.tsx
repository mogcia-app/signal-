...
const ReelAnalyticsStats: React.FC<ReelAnalyticsStatsProps> = ({
  ...
  // リール専用フィールドの統計
  const totalReelInteractionCount = reelData.reduce(
    (sum, data) => sum + (Number(data.reelInteractionCount) || 0),
    0
  );
  const avgReelReachFollowerPercent =
    reelData.length > 0
      ? reelData.reduce((sum, data) => sum + (Number(data.reelReachFollowerPercent) || 0), 0) /
        reelData.length
      : 0;
  const avgReelInteractionFollowerPercent =
    reelData.length > 0
      ? reelData.reduce(
          (sum, data) => sum + (Number(data.reelInteractionFollowerPercent) || 0),
          0
        ) / reelData.length
      : 0;

  const totalReelReachSourceProfile = reelData.reduce(
    (sum, data) => sum + (Number(data.reelReachSourceProfile) || 0),
    0
  );
  const totalReelReachSourceReel = reelData.reduce(
    (sum, data) => sum + (Number(data.reelReachSourceReel) || 0),
    0
  );
  const totalReelReachSourceExplore = reelData.reduce(
    (sum, data) => sum + (Number(data.reelReachSourceExplore) || 0),
    0
  );
  const totalReelReachSourceSearch = reelData.reduce(
    (sum, data) => sum + (Number(data.reelReachSourceSearch) || 0),
    0
  );
  const totalReelReachSourceOther = reelData.reduce(
    (sum, data) => sum + (Number(data.reelReachSourceOther) || 0),
    0
  );

  const totalReelReachedAccounts = reelData.reduce(
    (sum, data) => sum + (Number(data.reelReachedAccounts) || 0),
    0
  );

  const avgReelSkipRate =
    reelData.length > 0
      ? reelData.reduce((sum, data) => sum + (Number(data.reelSkipRate) || 0), 0) / reelData.length
      : 0;
  const avgReelNormalSkipRate =
    reelData.length > 0
      ? reelData.reduce((sum, data) => sum + (Number(data.reelNormalSkipRate) || 0), 0) /
        reelData.length
      : 0;

  const totalReelPlayTime = reelData.reduce(
    (sum, data) => sum + (Number(data.reelPlayTime) || 0),
    0
  );
  const avgReelPlayTime =
    reelData.length > 0
      ? reelData.reduce((sum, data) => sum + (Number(data.reelAvgPlayTime) || 0), 0) /
        reelData.length
      : 0;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-white border border-gray-200 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-2xl font-bold text-gray-800">
                    {totalReelInteractionCount.toLocaleString()}
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-white border border-gray-200 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-2xl font-bold text-gray-800">
                    {avgReelReachFollowerPercent.toFixed(1)}%
                  </div>
            ...
            <div className="bg-white border border-gray-200 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-2xl font-bold text-gray-800">
                    {avgReelInteractionFollowerPercent.toFixed(1)}%
                  </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="bg-white border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-gray-800">
                  {totalReelReachSourceProfile.toLocaleString()}
                </div>
          ...
          <div className="bg-white border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-gray-800">
                  {totalReelReachSourceReel.toLocaleString()}
                </div>
          ...
          <div className="bg-white border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-gray-800">
                  {totalReelReachSourceExplore.toLocaleString()}
                </div>
          ...
          <div className="bg-white border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-gray-800">
                  {totalReelReachSourceSearch.toLocaleString()}
                </div>
          ...
          <div className="bg-white border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-gray-800">
                  {totalReelReachSourceOther.toLocaleString()}
                </div>
      ...
      <div className="mb-6">
        ...
        <div className="bg-white border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-gray-800">
                {totalReelReachedAccounts.toLocaleString()}
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-white border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-2xl font-bold text-gray-800">
                  {avgReelNormalSkipRate.toFixed(1)}%
                </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-gray-800">
                {totalReelPlayTime.toLocaleString()}
              </div>
