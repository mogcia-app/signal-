...
export function generateDynamicAdvice(
  ...
  const monthlyGrowth = followerGain / (planPeriod === '1ヶ月' ? 1 : planPeriod === '3ヶ月' ? 3 : 6);
  ...
  if (currentFollowers < 100) {
    advice.push({
      ...
      title: '小規模フォロワー基盤の構築',
      content: 'フォロワー数100人未満の段階では、一貫した投稿頻度とエンゲージメント向上に集中しましょう。質の高いコンテンツを継続的に投稿し、既存フォロワーとの関係を深めることが重要です。',
      priority: 'high',
      category: 'strategy'
    });
    ...
    advice.push({
      ...
      title: '成長期の戦略的アプローチ',
      content: '100-1000人のフォロワー層では、リールコンテンツの活用とハッシュタグ戦略の最適化が効果的です。トレンドに敏感に反応し、エンゲージメント率の向上を目指しましょう。',
      priority: 'high',
      category: 'growth'
    });
    ...
    advice.push({
      ...
      title: '拡大期のブランド構築',
      content: '1000人以上のフォロワー層では、ブランドアイデンティティの確立とストーリーテリングが重要です。一貫したビジュアルスタイルと独自の価値提供を心がけましょう。',
      priority: 'medium',
      category: 'strategy'
    });
  }
  ...
  if (planPeriod === '1ヶ月') {
    advice.push({
      ...
      title: '短期間での目標達成戦略',
      content: '1ヶ月での大幅な成長は困難です。現実的な目標設定（月10-20%の成長）と、高エンゲージメントコンテンツの集中投稿を推奨します。リール投稿を週3-4回に増やすことを検討してください。',
      priority: 'high',
      category: 'timing'
    });
    ...
    advice.push({
      ...
      title: '中期戦略での着実な成長',
      content: '3ヶ月期間では、バランスの取れた投稿戦略が効果的です。リール・フィード・ストーリーを組み合わせ、段階的なフォロワー獲得を目指しましょう。月間15-25%の成長が現実的です。',
      priority: 'medium',
      category: 'timing'
    });
    ...
    advice.push({
      ...
      title: '長期戦略での持続的成長',
      content: '6ヶ月期間では、持続可能な投稿ペースとコミュニティ構築に重点を置きましょう。月間10-15%の着実な成長を目指し、フォロワーとの長期的な関係構築を重視してください。',
      priority: 'low',
      category: 'timing'
    });
  }
  ...
  if (simulationResult) {
    ...
    if (feasibilityLevel === 'very_challenging' || feasibilityLevel === 'challenging') {
      advice.push({
        id: 'goal-adjustment',
        title: '目標の現実的な調整',
        content: '現在の目標は非常に挑戦的です。期間の延長または目標値の段階的調整を検討することをお勧めします。まずは月間10-15%の成長を目指し、基盤を固めてから次のステップに進みましょう。',
        priority: 'high',
        category: 'strategy'
      });
    }
    ...
    if (monthlyPosts > 30) {
      advice.push({
        ...
        content: '月30回以上の投稿は高負荷です。コンテンツの質を保つため、事前準備とスケジュール管理を徹底しましょう。バッチ作業で効率化を図ることをお勧めします。',
        priority: 'high',
        category: 'timing'
      });
    }
  ...
  // 成長率に基づくアドバイス
  if (monthlyGrowth > 0.3) { // 30%以上の成長率
    advice.push({
      id: 'aggressive-growth',
      title: '積極的成長戦略の注意点',
      content: '月間30%以上の成長は非常に積極的です。一時的な成長よりも持続可能な成長を重視し、フォロワーの質を維持することに注意してください。',
      priority: 'high',
      category: 'growth'
    });
  } else if (monthlyGrowth < 0.05) { // 5%未満の成長率
    advice.push({
      id: 'conservative-growth',
      title: '保守的成長戦略の最適化',
      content: '月間5%未満の成長は保守的です。より積極的なコンテンツ戦略やエンゲージメント向上施策を検討し、成長ペースを上げることをお勧めします。',
      priority: 'medium',
      category: 'growth'
    });
  }
  ...
  // コンテンツ戦略のアドバイス
  if (targetFollowers > currentFollowers * 2) { // 2倍以上の成長目標
    advice.push({
      id: 'content-diversification',
      title: 'コンテンツ多様化の必要性',
      content: '大幅な成長目標には多様なコンテンツ戦略が必要です。リール、フィード、ストーリーを効果的に組み合わせ、各フォーマットの特性を活かした投稿を心がけましょう。',
      priority: 'medium',
      category: 'content'
    });
  }
  ...
  advice.push({
    ...
    content: 'フォロワーとの積極的な交流を心がけましょう。コメントへの返信、ストーリーでの質問、フォロワーの投稿へのリアクションなど、双方向のコミュニケーションが成長の鍵となります。',
    priority: 'medium',
    category: 'engagement'
  });
...
// アドバイスのカテゴリ別フィルタリング
export function filterAdviceByCategory(advice: DynamicAdvice[], category: DynamicAdvice['category']): DynamicAdvice[] {
  return advice.filter(item => item.category === category);
}

// 優先度別フィルタリング
export function filterAdviceByPriority(advice: DynamicAdvice[], priority: DynamicAdvice['priority']): DynamicAdvice[] {
