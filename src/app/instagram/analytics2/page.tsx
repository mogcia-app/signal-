...
function InstagramAnalyticsContent() {
  ...
  const handleSaveAnalytics = async () => {
    ...
    try {
      ...
      const engagementRate =
        reach > 0 ? (((likes + comments + shares) / reach) * 100).toFixed(2) : "0";
  ...
  const avgEngagementRate =
    analyticsData.length > 0
      ? analyticsData.reduce((sum, data) => sum + (data.engagementRate || 0), 0) /
  ...
  return (
    <>
      <SNSLayout customTitle="投稿分析" customDescription="投稿の分析データを入力・管理します">
