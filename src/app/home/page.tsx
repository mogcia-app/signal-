...
export default function HomePage() {
  ...
  const sendAdvisorMessage = async (rawMessage: string) => {
    ...
    try {
      ...
      setAdvisorSuggestedQuestions(
        Array.isArray(result.data.suggestedQuestions)
          ? result.data.suggestedQuestions.map((q: unknown) => String(q || "").trim()).filter(Boolean).slice(0, 3)
          : []
      );
    } catch (error) {
  ...
  const getTimelineTypeBadgeClass = (type?: string): string => {
    if (type === "reel") {return "border-violet-200 bg-violet-50 text-violet-700";}
    if (type === "story") {return "border-emerald-200 bg-emerald-50 text-emerald-700";}
                            ...
                            <div className="flex gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-start justify-between gap-2">
                                  ...
                                  <span className={`text-[11px] px-2 py-0.5 whitespace-nowrap ${postTypeBadgeClass}`}>
                                    {postTypeLabel}
                                  </span>
                                </div>
                  ...
                  <aside className="lg:col-span-6 border border-gray-200 bg-white p-6 sm:p-8 space-y-4">
                    ...
                    <div className="border border-gray-200 bg-white p-3 space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-xs font-semibold text-gray-800">商品・サービス（任意）</p>
                    ...
                    <div className="border border-orange-200 bg-orange-50/40 p-3 space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-xs font-semibold text-gray-800">テーマ（任意）</p>
                        {homePostPrompt.trim() ? (
                          <button
                            type="button"
            ...
            {!showPlanGateLoader && currentPlan && !showHomePlanForm && (
              <div className="mb-4 space-y-2">
                ...
                {(() => {
                  ...
                  const monthlyFocusKpi = getAimLabelFromPurpose(String(currentPlan.operationPurpose || ""));
                  const startDateText =
                  ...
                  return (
                    <div className="relative overflow-hidden border border-orange-200 bg-white p-5">
                      <div className="relative">
                        ...
                        <div className="mb-4 overflow-hidden border border-orange-200 bg-[#fffaf5] shadow-sm">
                          <div className="flex items-center justify-between border-b border-orange-200 bg-gradient-to-r from-orange-500 to-amber-500 px-4 py-2">
                            <div className="flex items-center gap-2">
                          ...
                          <div className="px-4 py-3">
                            <p className="text-sm leading-7 text-black">
                              <span className="text-xs text-gray-500">今月の目的</span>
                              <span className="mx-1 text-black">
                                {String(currentPlan.operationPurpose || "投稿の目的")}
                              </span>
                              <span className="text-xs text-gray-500">重点KPI</span>
                              <span className="mx-1 inline-flex items-center text-black">
                                {monthlyFocusKpi}
                              </span>
                            </p>
                          </div>
